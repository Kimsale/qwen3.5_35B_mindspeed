...
**约束**: 不改模型结构/MoE 路由/专家数量;LoRA-only;数学一致。当前主目标是提高 WPS，严格 HBM 55-60G 只作为资源利用参考。
...
4. ⏳ **性能复测**: 按报告口径(跳过前 10 step warmup)跑候选配置,采集 step/WPS/AICORE/HBM/Power。主对比基线改为 EP8 LLM pack 最优配置: `pack rc_off, mbs=1`, last 40 steps `2111.4 WPS`,约 `3.6s/iter`,约 `40GB/card HBM`;`pad1536_nosync` 仅保留为 strict-HBM 参考点。
...
- **合理目标**是"在 mbs1 下尽量榨干吞吐"，HBM 55-60G 只作为资源占用参考。通信掩盖类优化只有在它能压低 step 耗时时才有价值，不能为了抬 HBM 牺牲 WPS。
...
后续在 `feat/qwen35-audio-pipeline-experts-overlap` 分支实现了手动 Pipeline Experts overlap，而不是继续停留在策略阶段。实现包含 chunk 化 dispatch/combine、多 stream 异步 AllToAll、专家 GMM 流水化和 MoE phase timing。稳定配置 `mbs1_pipeline_pad1536_nosync_80.yaml` 在 `172.29.226.188` 完成 `80/80` step。

实测结论与预期不同：Pipeline run post-warmup `645.5 WPS`、`8.589s/step`、AICORE mean `12.04%`、HBM mean `56.6GB/card`。相对 EP8 LLM pack `rc_off, mbs=1` 的 `2111.4 WPS`，只达到 `30.6%`，属于性能回退。

直接原因是 chunk 后专家计算太小，`pipeline_compute` 约 `7.0ms`，无法覆盖 `pipeline_wait_dispatch` 约 `10.4ms`；同时 pipeline 调度、A2A work 管理和额外同步使 forward 平均时间涨到 `5727.8ms`。因此后续主线应回到 pack/有效 token 密度和 rank-aligned pack mbs2，而不是继续放大当前 Pipeline Experts 实现。
