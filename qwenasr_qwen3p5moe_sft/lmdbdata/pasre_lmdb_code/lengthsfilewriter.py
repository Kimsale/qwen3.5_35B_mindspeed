...
from file_io.file_io import FileIO, FileReader, FileWriter
...
class LengthsFileReader(LengthsIndexedFileIO, FileReader):
    ...
    def read(self, index: int) -> int:
        ...
        return np.frombuffer(
            ...
            dtype=self._dtype,
            count=1,
            offset=index * self.itemsize()).item()
    ...
    def chunk_iter(self, start: int, end: int) -> Iterable[int]:
...
def lengths_bin_generator(length_list,bin_file):
    writer = LengthsFileWriter(bin_file)
    writer.open()
    writer.writes(length_list)
    writer.flush()
